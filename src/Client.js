    ...
    async initialize(auth_method) {
        ...
        if (needAuthentication) {
            ...
            if (auth_method == 1) {
                ...
                const CODE_BUTTON = 'span[role]';

                await page.waitForSelector(CODE_BUTTON, { timeout: 10000 });
                await page.evaluate(function (selectors) {
                    const code_button = document.querySelector(selectors.CODE_BUTTON);
                    if (code_button) code_button.click();
                },
                // Set selectors
                {
                    CODE_BUTTON
                ...
                const INPUT_FORM = '/html/body/div[1]/div/div/div[3]/div[1]/div/div[3]/div[2]/div/div';
                const INPUT_NUMBER = 'form > input';
                const NEXT_BUTTON = '#app > div > div > div.landing-window > div.landing-main > div > div > div > div[role]';

                await page.waitForSelector(INPUT_NUMBER, { timeout: 10000 });

                // Set focus and clear input number form
                const input_number = await page.$x(INPUT_FORM);
                await input_number[0].click({ clickCount: 3 });
                await page.keyboard.press('Backspace');

                const clientNumber = this.authStrategy.clientId;

                await page.keyboard.type('+' + clientNumber);
                ...
                const CODE = '#app > div > div > div.landing-window > div.landing-main > div > div > div._2I5ox > div > div > div';
                ...
                await page.exposeFunction('codeChanged', async (code) => {
                    ...
                    */
                    ...
                    let code_parsed = code.toString().replace(regExp, '');
                    this.emit(Events.CODE_RECEIVED, code_parsed);
                });

                await page.evaluate(function (selectors) {
                    // CODE PARSER
                    const res = Array.from(document.querySelector(selectors.CODE).children, ({ textContent }) => textContent.trim()).filter(Boolean);
                    window.codeChanged(res);

                    var el_mut = window.getElementByXpath('//*[@id="app"]/div/div/div[3]/div[1]/div/div/div[2]');
                    const observer = new MutationObserver(function (mutations_list) {
                        mutations_list.forEach(function (mutation) {
                            mutation.removedNodes.forEach(function (/*removed_node*/) {
                                setTimeout(() => {
                                    //If code not exist
                                    let code_element = document.querySelector(selectors.CODE);
                                    if (code_element == null) {
                                        const REPLAY_CODE = '//*[@id="app"]/div/span/div/div/div/div/div/div/div[3]/div/div';
                                        const replayCode_button = window.getElementByXpath(REPLAY_CODE);
                                        if (replayCode_button) replayCode_button.click();
                                    }
                                }, 2000);
                    ...
                    observer.observe(el_mut, { attributes: true, subtree: true, childList: true, characterData: true });
                },
                // Set selectors
                {
                    CODE
